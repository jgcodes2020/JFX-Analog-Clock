...
import jgcodes.fxanalogclock.audio.ResourceClipPlayer;
...
public class ClockDrawer extends AnimationTimer {
  ...
  private ResourceClipPlayer clipPlayer = null;
  ...
  public ClockDrawer(Canvas canvas) {
    ...
    try {
      clipPlayer = new ResourceClipPlayer("clock-tick.wav");
      hasSound = true;
    } catch (ResourceException e) {
      System.err.println("Could not locate clock-tick.wav");
      muted = true;
      hasSound = false;
      e.printStackTrace();
    } catch (IOException | UnsupportedAudioFileException | LineUnavailableException e) {
      System.err.println("Can't play sound! Muting...");
      muted = true;
      hasSound = false;
      e.printStackTrace();
    }
  ...
  private int lastSecond = LocalTime.now().getSecond();
  
  @Override
  public void handle(long animTime) {
    LocalTime time = LocalTime.now();
    if (time.getSecond() != lastSecond) {
      if (!isMuted()) clipPlayer.play();
  ...
  public void setMuted(boolean muted) {
    if (hasSound) this.muted = muted;
    else throw new UnsupportedOperationException("Drawer does not have access to sound");
  }
}
